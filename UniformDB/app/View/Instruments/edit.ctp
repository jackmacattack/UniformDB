<div class="instruments form">
<?php echo $this->Form->create('Instrument'); ?>
	<fieldset>
		<legend><?php echo __('Edit Instrument'); ?></legend>
	<?php
		echo $this->Form->inputs();
	?>
	</fieldset>
<?php echo $this->Form->end(__('Submit')); ?>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Instrument'), array('action' => 'add')); ?> </li>	
		<li><?php echo $this->Form->postLink(__('Delete Instrument'), array('action' => 'delete', $this->Form->value('Instrument.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('Instrument.id'))); ?></li>
		<li><?php echo $this->Html->link(__('Cancel'), array('action' => 'index')); ?></li>
	</ul>
</div>
